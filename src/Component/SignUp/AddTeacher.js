...
export default function AddTeacher() {
    ...
    async function register() {
        if (name.length === 0 || email.length === 0 || me.length === 0 || mobile.length != 10 ||
            pwd.length < 8 || pwd.length > 16 || pwd.length !== cpwd.length || sub == "0" ||
            deg.length === 0 || exp.length === 0 || ext.length === 0) {
            console.log(" error");
            seterr(true)
            seterr1(true)
            seterr2(true)
            ...
            await axios.post('http://localhost:3001/teacher',
                {
                    name: name,
                    email: email,
                    mobile: mobile,
        ...
        <>
            ...
            <Modal show={show} fullscreen onHide={handleClose}>
                <Modal.Header closeButton className="bg-success">
                    <Modal.Title><h1 className="float-center display-2">Teacher Registration Form</h1 ></Modal.Title>
                </Modal.Header>
                <Modal.Body className="container-t">
                    ...
                    <Container className="mt-5">
                        ...
                        <Row className="my-3">
                            <Col lg={4} className="text-end">
                                <label className="lbl-txt"> Which subject you teach :- </label>
                            </Col>
                            <Col lg={3}>
                            ...
                            <Col lg={2} className="text-end">
                                <label className="lbl-txt"> Experience :- </label>
                            </Col>
                            <Col lg={3}>
                                <input type="text" placeholder="Your Experience..."
                                    value={exp} onChange={(e) => setexp(e.target.value)}
                                    onBlurCapture={(e) => e.target.value.length === 0 ? seterr5(true) : seterr5(false)}
                                ></input>
                                {err5 ?
                                    <span style={{ color: '#f00' }}>This field is necesary.</span> :
                                    <span></span>}
                        ...
                        <Row className="my-3">
                            <Col lg={4} className="text-end">
                                <label className="lbl-txt"> Degree :- </label>
                            </Col>
                            <Col lg={8}>
                                <input type="text" placeholder="Your Education Qualification..."
